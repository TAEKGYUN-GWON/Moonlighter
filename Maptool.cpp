...
void Maptool::Render()
{
	...
	GRAPHICMANAGER->DrawTextD2D(Vector2(WINSIZEX / 2, 30), buffer, 20, 1.0f, ColorF::Red, DWRITE_TEXT_ALIGNMENT_LEADING, L"맑은고딕", false);

	sprintf_s(buffer, "index Y : %d", (_ptMouse.y - (int)CAMERA->GetPosition().y) / TILEHEIGHT);
	GRAPHICMANAGER->DrawTextD2D(Vector2(WINSIZEX / 2, 60), buffer, 20, 1.0f, ColorF::Red, DWRITE_TEXT_ALIGNMENT_LEADING, L"맑은고딕", false);
	...
	GRAPHICMANAGER->DrawTextD2D(Vector2(WINSIZEX / 2, 90), buffer, 20, 1.0f, ColorF::Red, DWRITE_TEXT_ALIGNMENT_LEADING, L"맑은고딕", false);
}
