...
void Maptool::Render()
{
	...
	for (int i = 0; i < 25; ++i)
	{
		for (int j = 0; j < 33; ++j)
		{
			...
			GRAPHICMANAGER->DrawTextD2D(_tiles[index]->GetTrans()->GetPos() + Vector2(-(TILEWIDTH / 2) + 2, TILEHEIGHT / 7), buffer, 10, 1.0f, ColorF::Yellow, DWRITE_TEXT_ALIGNMENT_LEADING, L"맑은고딕", true);
		}
	}
	...
	GRAPHICMANAGER->DrawTextD2D(Vector2(100, 200), buffer, 20, 1.0f, ColorF::Red);
...
void Maptool::Load()
{
	...
	else MessageBox(_hWnd, "없쒀", str.c_str(), MB_OK);
