...
void Maptool::Render()
{
	...
	for (int i = 0; i < TILENUMX * TILENUMY; i++)
	{
		if (_tiles[i]->GetAttribute() == "Wall") _tiles[i]->GetComponent<Sprite>()->SetFillRect(true);
		else if (_tiles[i]->GetAttribute() != "Wall") _tiles[i]->GetComponent<Sprite>()->SetFillRect(false);

		sprintf_s(buffer, "%d", i);
		//GRAPHICMANAGER->DrawTextD2D(_tiles[i]->GetTrans()->GetPos() + Vector2(-(TILEWIDTH / 2) + 2, TILEHEIGHT / 7), buffer, 10, 1.0f, ColorF::Yellow);
	}
	...
	//{
	//	int cullX = CAMERA->GetPosition().x;
	//	int cullY = CAMERA->GetPosition().y;

	//	if (_index < 0 || _index >= TILENUMX * TILENUMY) continue;
	//	else child->SetAllowsRender(false);

	//	if(child->GetTrans()->pos < CAMERA->GetPosition())

	//	child->SetAllowsRender(true);
	//	
	//	child->Render();
	//}
...
#pragma region TileSetting
	_mTileList.insert(make_pair(("build_fountain"), tagTile().Clone("build_fountain", "Wall", false, 1, 1, PIVOT::BOTTOM, Vector2(2, 1), Vector2(3, 1))));
	...
	_mTileList.insert(make_pair(("build_Top1"), tagTile().Clone("build_Top1", "Wall", false, 1, 1, PIVOT::BOTTOM, Vector2(6, 10), Vector2(11, 10))));
	...
	_mTileList.insert(make_pair(("door"), tagTile().Clone("door", "Wall", true, 5, 1, PIVOT::BOTTOM, Vector2(3, 1), Vector2(2, 1), Vector2(0, 1), Vector2(2, 1))));
	_mTileList.insert(make_pair(("doordown"), tagTile().Clone("doordown", "Wall", true, 5, 1, PIVOT::BOTTOM, Vector2(3, 3), Vector2(2, 1), Vector2(0, 3), Vector2(2, 1))));
	_mTileList.insert(make_pair(("dungeonLobby_entryStructure"), tagTile().Clone("dungeonLobby_entryStructure", "Wall", false, 1, 1, PIVOT::BOTTOM, Vector2(7, 1), Vector2(4, 1), Vector2(-2, 1), Vector2(4, 1))));
	...
	_mTileList.insert(make_pair(("build_Board"), tagTile().Clone("build_Board", "Wall", false, 1, 1, PIVOT::BOTTOM, Vector2(4, 1), Vector2(7, 1))));
	...
	_mTileList.insert(make_pair(("build_Bottom2"), tagTile().Clone("build_Bottom2", "Wall", false, 1, 1, PIVOT::BOTTOM, Vector2(1, 1), Vector2(1, 1))));
	...
	_mTileList.insert(make_pair(("tree"), tagTile().Clone("tree", "Wall", true, 4, 1, PIVOT::BOTTOM, Vector2(2, 1), Vector2(3, 1))));
	...
	_mTileList.insert(make_pair(("dungeonLobby_middleRighttRock"), tagTile().Clone("dungeonLobby_middleRighttRock", "Wall", false, 1, 1, PIVOT::BOTTOM, Vector2(2, 1), Vector2(3, 1))));
...
void Maptool::SetAttribute(int curIdx, Vector2 StartPos, Vector2 size, Vector2 StartPos2, Vector2 size2, string attribute)
{
	...
	for (int i = 0; i < size.y; ++i)
	{
		for (int j = 0; j < size.x; ++j)
		{
			...
			_tiles[start + j + (TILENUMX * i)]->SetId(_id);
		}
	}
